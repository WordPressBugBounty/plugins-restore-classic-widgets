...
class restore_classic_widgetsrestore_classic_widgets_show_more_plugins
{
    ...
    public function enqueue_scripts()
    {
        ...
        $parts = explode("/", rtrim($plugin_path, "/"));
        $plugin_slug = reset($parts);
        $plugin_url = plugins_url() . "/" . $plugin_slug;
        $style_handle = substr(
            md5_file(restore_classic_widgets_dir_path(__FILE__) . "/more.css"),
            0,
            8
        );
        $script_handle = $style_handle . "-js"; // Guardamos o handle do script numa variável
        wp_enqueue_style(
            $style_handle,
            plugin_dir_url(__FILE__) . "/more.css",
            array(),             // Added empty array for dependencies (if none)
            RESTORE_CLASSIC_WIDGETSVERSION // Explicitly set version for cache busting
        );
        wp_register_script(
            $script_handle, // Usamos a variável do handle aqui
            plugin_dir_url(__FILE__) . "/more.js",
            ["jquery"],          // Dependencies
            RESTORE_CLASSIC_WIDGETSVERSION, // Explicitly set version for cache busting
            false                // default value, or true if you want to load in footer
        );
        wp_enqueue_script($script_handle); // E aqui
        wp_register_script(
            "bill-js-toast24",
            $plugin_url . "/assets/js/jquery.toast.js",
            array(),             // Corrected: Dependencies should be an array (empty if none)
            RESTORE_CLASSIC_WIDGETSVERSION, // Explicitly set version for cache busting
            true                 // Added: Load in footer (common for UI scripts like this)
        );
        wp_enqueue_script("bill-js-toast24");
        $ajax_params = [
            'ajax_url' => admin_url('admin-ajax.php'),
            'nonce'    => wp_create_nonce('restore_classic_widgets_install_plugin_class')
        ];
        wp_localize_script($script_handle, 'restore_classic_widgets_install_params', $ajax_params);
    }
...
?>
        <div style="padding-right:20px;">
            ...
            <table style="margin-right:20px; border-spacing: 0 25px; " class="widefat" cellspacing="0" id="restore_classic_widgets_class_install-more-plugins-table">
                <tbody class="restore_classic_widgets_class_install-more-plugins-body">
                    ...
                    for ($i = 0; $i < $total; $i++) {
                        ...
                        echo "<h3>" . esc_html($plugins_to_install[$i]["Name"]) . "</h3>";
                        echo esc_html($plugins_to_install[$i]["Description"]);
                        echo "<br>";
                        echo "</td>";
                        echo '<td style="max-width:140px; max-height:140px;" >';
            ...
            <?php
            wp_nonce_field("restore_classic_widgets_install_plugin_class", "nonce_install");
            $plugin_path = trailingslashit(dirname(plugin_basename(__FILE__)));
...
$show_more_plugins = new restore_classic_widgetsrestore_classic_widgets_show_more_plugins();
class restore_classic_widgetsPluginInstaller
{
    public function __construct()
    {
        add_action("wp_ajax_restore_classic_widgets_install_plugin2", array($this, "restore_classic_widgets_install_plugin2"));
    }
    public function restore_classic_widgets_install_plugin2()
    {
        if (!isset($_POST["nonce"])) {
            ...
            if (
                !wp_verify_nonce($nonce, "restore_classic_widgets_install_plugin_class") &&
                !wp_verify_nonce($nonce, "restore_classic_widgets_install")
            ) {
                wp_die("invalid nonce");
            }
        } 
        ...
        $allowed_slugs = [
            "database-backup", "bigdump-restore", "easy-update-urls", "hide-site-title",
            "s3cloud", "toolsfors3", "antihacker", "stopbadbots", "wptools",
            "recaptcha-for-all", "wp-memory", "disable-wp-sitemap",
            "restore_classic_widgets", "antibots", "site-checkup"
        ];
        ...
        $skin = new class(['api' => $api]) extends \WP_Upgrader_Skin
        {
...
$plugin_installer = new restore_classic_widgetsPluginInstaller();